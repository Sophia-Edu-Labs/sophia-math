...
#only("2")[#voiceover("Option a) $f(x)$ ist falsch. Wenn $f(x)$ und $g(x)$ Inverse sind, sollte ihre Komposition nicht eines der ursprünglichen Funktionen ergeben.")]
#only("3")[#voiceover("Option b) $g(x)$ ist ebenfalls falsch aus dem gleichen Grund. Die Komposition von inversen Funktionen ergibt nicht eine der ursprünglichen Funktionen.")]
#only("4")[#voiceover("Option c) $x$ ist korrekt! Wenn Sie eine Funktion mit ihrem Inversen zusammensetzen, erhalten Sie die Identitätsfunktion, die einfach $x$ ist.")]
...
#only("4")[#voiceover("Wir nehmen an, dass $f(a)$ gleich $b$ und $g(b)$ gleich $a$ ist. Das bedeutet, dass $f$ und $g$ Inverse sind.")]
...
#only("7")[#voiceover("Und wir wissen, dass $f(a)$ gleich $b$ ist.")]
...
#text(size: 30pt, weight: "bold")[Schritt-für-Schritt-Lösung]
#v(40pt)
#only("1-")[Schritt 2: Funktionen zusammensetzen 🧩 (Fortsetzung)]
#v(10pt)
