...
#v(10pt)
// Vom Anfang bis Abschnitt zwei ist die Option schwarz. Ab Abschnitt 3 bis zum Ende wird sie dann rot, weil sie falsch ist.
#only("-2")[b) oszillieren]#only("3-")[#text(fill:red)[b) oszillieren]]
#v(10pt)
// Vom Anfang bis Abschnitt drei ist die Option schwarz. Ab Abschnitt 4 bis zum Ende wird sie dann grün, weil sie richtig ist.
...
#only("1")[#voiceover("Lassen Sie uns dies Schritt für Schritt erklären. Eine Folge a_n divergiert ins Unendliche, wenn für jedes M größer als 0 ein N existiert, so dass für alle n größer oder gleich N, a_n größer als M ist.")]
...
#only("1")[#voiceover("Schauen wir uns ein Beispiel an. Betrachten Sie die Folge a_n gleich n Quadrat. Hier ist ein Diagramm der ersten paar Glieder.")]
...
#only("4")[#voiceover("Somit, egal welches M wir wählen, können wir immer ein N finden, so dass alle nachfolgenden Glieder größer als M sind. Daher divergiert die Folge a_n gleich n Quadrat ins Unendliche.")]
]
]
...
#v(20pt)
- Formal, für jedes $M > 0$, gibt es ein $N$, so dass für alle $n >= N$, $a_n > M$. 🎓
#v(20pt)
