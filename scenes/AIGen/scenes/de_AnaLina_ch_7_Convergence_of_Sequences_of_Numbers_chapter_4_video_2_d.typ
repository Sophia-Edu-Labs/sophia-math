...
- Eine Folge $a_n$ divergiert ins Unendliche, wenn:
  - Für jedes $M > 0$ existiert ein $N$, so dass für alle $n \geq N$ gilt: $a_n > M$. 🚀
#v(20pt)  
- Das bedeutet:
...
#only("1")[#voiceover("Lassen Sie uns dies Schritt für Schritt aufschlüsseln. Eine Folge a_n divergiert ins Unendliche, wenn für jedes M größer als 0 ein N existiert, so dass für alle n größer oder gleich N gilt: a_n ist größer als M.")]
...
- Dann gilt für alle $n >= N$, $a_n = n^2 >= N^2 > M$. ✅
- Somit divergiert $a_n$ ins Unendliche. 🚀
#only("1")[#voiceover("Betrachten wir ein Beispiel. Betrachten Sie die Folge a_n gleich n Quadrat. Hier ist ein Diagramm der ersten paar Glieder.")]
#only("2")[#voiceover("Um zu zeigen, dass diese Folge ins Unendliche divergiert, wählen wir ein beliebiges M größer als 0. Wir müssen ein N finden, so dass für alle n größer oder gleich N gilt: a_n ist größer als M.")]
#only("3")[#voiceover("Wir können N als die Decke der Quadratwurzel von M wählen. Dann gilt für alle n größer oder gleich N, dass a_n gleich n Quadrat ist, was größer oder gleich N Quadrat ist, was wiederum größer als M ist.")]
#only("4")[#voiceover("Somit können wir, egal welches M wir wählen, immer ein N finden, so dass alle nachfolgenden Glieder größer als M sind. Daher divergiert die Folge a_n gleich n Quadrat ins Unendliche.")]
]
]
...
#v(20pt)
- Formal bedeutet das, dass für jedes $M > 0$ ein $N$ existiert, so dass für alle $n >= N$ gilt: $a_n > M$. 🎓
#v(20pt)
...
#only("2")[#voiceover("Formal bedeutet das, dass für jedes M größer als 0 ein N existiert, so dass für alle n größer oder gleich N gilt: a_n ist größer als M.")]
