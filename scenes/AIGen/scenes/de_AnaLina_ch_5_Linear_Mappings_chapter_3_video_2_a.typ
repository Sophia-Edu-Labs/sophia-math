...
#only("2")[#voiceover("a) $w = T(v)$ für ein $v$ in $V$. Das Bild einer linearen Abbildung $T$ ist tatsächlich die Menge aller Vektoren $w$, die durch Anwenden von $T$ auf einen Vektor $v$ im Definitionsbereich $V$ erhalten werden können.")]
// Sehr kurz über b sprechen und warum es falsch ist
#only("3")[#voiceover("b) $w = v$ für ein $v$ in $V$ ist falsch. Dies würde bedeuten, dass das Bild dasselbe wie der Definitionsbereich ist, was für eine lineare Abbildung nicht unbedingt zutrifft.")]
// Sehr kurz über c sprechen und warum es falsch ist
#only("4")[#voiceover("c) $w = 0$ ist auch falsch. Während der Nullvektor Teil des Bildes sein könnte, ist er nicht das einzige Element. Das Bild kann viele andere Vektoren enthalten.")]
// Sehr kurz über d sprechen und warum es falsch ist
#only("5")[#voiceover("Und d) $w = -T(v)$ für ein $v$ in $V$ ist ebenfalls falsch. Dies würde bedeuten, dass das Bild aus den Negativen der abgebildeten Vektoren besteht, was nicht die Definition des Bildes ist.")]
]
...
#v(20pt)
#only("3-")[Also für jedes $mat(a; b)$ in $RR^2$ existiert ein $mat(x; y)$, sodass $T(mat(x; y)) = mat(a; b)$.]
