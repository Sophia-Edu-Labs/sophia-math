...
#only("1")[
#voiceover("Lassen Sie uns die richtige Lösung überprüfen, um zu sehen, ob die Folge a_n gleich n Quadrat ins Unendliche divergiert.")
]
#only("2")[
...
#only("2-")[2. Wählen Sie $N = ceil(sqrt(M))$. Dann gilt für alle $n >= N$, dass $n >= sqrt(M)$, was impliziert, dass $n^2 >= M$ ist.]
#only("3-")[3. Somit existiert für jedes $M > 0$ ein $N$ (nämlich $ceil(sqrt(M))$), so dass für alle $n >= N$, $a_n = n^2 > M$ ist.]
...
#only("2")[
#voiceover("Nun wählen wir N als die Decke der Quadratwurzel von M. Dann gilt für alle n größer oder gleich N, dass n größer oder gleich der Quadratwurzel von M ist, was impliziert, dass n Quadrat größer oder gleich M ist.")
]
#only("3")[
...
#v(20pt)
#only("3-")[- Dies liegt daran, dass $n^2$ ohne Grenze wächst, wenn $n$ zunimmt. ⏫]
#only("1")[
...
#only("3")[
#voiceover("Der Grund dafür ist, dass n Quadrat ohne Grenze wächst, wenn n zunimmt.")
]
]