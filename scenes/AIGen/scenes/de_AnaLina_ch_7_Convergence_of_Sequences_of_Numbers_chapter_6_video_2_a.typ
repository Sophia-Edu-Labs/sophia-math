...
#only("5")[
#voiceover("Option d) ist falsch. Obwohl eine ins Unendliche divergierende Folge sicherlich nicht für alle großen $n$ gleich einem festen $M$ ist, reicht diese Bedingung allein nicht aus, um die Divergenz ins Unendliche zu garantieren.")
]
]
...
#v(20pt)
#only("5-")[3️⃣ Für alle $n \geq N$ gilt $n \geq "sqrt"(M)$, also $n^2 \geq M$.]
#v(20pt)
#only("6-")[∴ $a_n = n^2$ divergiert ins Unendliche. ∎]

#only("1")[
#voiceover("Lassen Sie uns dieses Konzept anhand eines Beispiels verstehen. Betrachten Sie die Folge $a_n$ gleich $n$ Quadrat.")
]
#only("2")[
#voiceover("Unser Ziel ist es zu zeigen, dass für jede positive reelle Zahl $M$ eine natürliche Zahl $N$ existiert, so dass $n$ Quadrat größer als $M$ für alle $n$ größer oder gleich $N$ ist.")
]
#only("3")[
#voiceover("Zuerst sei $M$ eine beliebige positive reelle Zahl.")
]
#only("4")[
#voiceover("Nun wählen Sie $N$ als eine beliebige ganze Zahl, die größer als die Quadratwurzel von $M$ ist. Dann ist $N$ Quadrat größer als $M$.")
]
#only("5")[
#voiceover("Für alle $n$ größer oder gleich $N$ haben wir $n$ größer oder gleich der Quadratwurzel von $M$, also ist $n$ Quadrat größer oder gleich $M$.")
]
#only("6")[
#voiceover("Daher divergiert die Folge $a_n$ gleich $n$ Quadrat ins Unendliche. Dies vervollständigt den Beweis.")
]
]
...
#only("1")[
#voiceover("Dieses Diagramm visualisiert die Divergenz der Folge $a_n$ gleich $n$ Quadrat ins Unendliche.")
]
#only("2")[
...
#only("4")[
#voiceover("Wie Sie sehen können, ist für jedes $M$ die Folge $a_n$ größer als $M$ für alle $n$ größer oder gleich dem entsprechenden $N$. Dies veranschaulicht die Divergenz ins Unendliche.")
]
]