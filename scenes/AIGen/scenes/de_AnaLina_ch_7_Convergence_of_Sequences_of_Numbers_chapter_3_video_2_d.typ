...
#only("1")[#voiceover("Das ist leider nicht korrekt. Die richtige Antwort ist...")]
// Erkläre kurz, warum b korrekt ist
...
#only("4")[#voiceover("c) gleich epsilon ist ebenfalls falsch. Der Unterschied muss nicht genau gleich epsilon sein, sondern kleiner als epsilon.")]
// Erkläre kurz, warum d falsch ist
#only("5")[#voiceover("Und d) ungleich epsilon ist ebenfalls falsch. Der Unterschied kann gleich epsilon sein und trotzdem die Definition erfüllen, solange er kleiner als epsilon ist.")]
...
#v(40pt)
#only("1-")[Lass uns die Definition aufschlüsseln:]
#v(20pt)
#only("2-")[1. Für jedes $epsilon > 0$: Wähle eine beliebige positive Zahl $epsilon$. 🔍]
#only("3-")[2. Es existiert ein $N$: Finde eine natürliche Zahl $N$. 🔢]
#only("4-")[3. So dass für alle $n >= N$: Betrachte alle natürlichen Zahlen $n$, die größer oder gleich $N$ sind. ⏰]
...
#only("1")[#voiceover("Gehen wir nun die Definition Schritt für Schritt durch.")]
...
#only("6")[#voiceover("Wenn wir für jedes $epsilon$ größer als 0 ein solches $N$ finden können, dann können wir sagen, dass die Folge $a_n$ gegen $L$ konvergiert.")]
]
...
#only("1")[
#voiceover("Hier ist eine Visualisierung der Konvergenzdefinition. Die blauen Punkte repräsentieren die Glieder der Folge $a_n = 1/n$. Die rote Linie ist der Grenzwert $L = 0$.")
]
#only("2")[
#voiceover("Wenn $n$ größer wird, nähern sich die Glieder immer mehr dem Wert 0. Für jedes gewählte epsilon können wir ein $N$ finden, so dass für alle $n$ größer als $N$ die Glieder innerhalb von epsilon um 0 liegen.")
]
#only("3")[
