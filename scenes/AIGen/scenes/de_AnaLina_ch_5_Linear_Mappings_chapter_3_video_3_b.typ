...
#only("1")[
#voiceover("Nicht ganz. Lass uns die richtige Lösung Schritt für Schritt gemeinsam durchgehen.")
]
]
...
#v(20pt)
#only("2-")[Damit $mat(a; b)$ im Bild von $T$ liegt, muss es $mat(x; y)$ geben, so dass $T(mat(x; y)) = mat(a; b)$.]
#v(20pt)
#only("3-")[Das bedeutet: $mat(x + y; x - y) = mat(a; b)$]
#v(20pt)
#only("4-")[Komponenten gleichsetzen: $x + y = a$ und $x - y = b$]
#v(20pt)
#only("5-")[Die Gleichungen addieren: $(x + y) + (x - y) = a + b$, was zu $2x = a + b$ vereinfacht]
#v(20pt)
#only("6-")[Daher ist das Bild von $T$ die Menge aller Vektoren $mat(a; b)$, so dass $a + b$ gerade ist.]
#only("1")[
#voiceover("Betrachten wir zunächst einen beliebigen Vektor (a; b) in R-2.")
]
#only("2")[
#voiceover("Damit dieser Vektor im Bild von T liegt, muss es einen Vektor (x; y) geben, so dass T von (x; y) gleich (a; b) ist.")
]
#only("3")[
...
#only("6")[
#voiceover("Das bedeutet, dass a plus b eine gerade Zahl sein muss, damit (a; b) im Bild von T liegt. Daher ist das Bild von T die Menge aller Vektoren (a; b), so dass a plus b gerade ist.")
]
]
...
#only("1")[
#voiceover("Hier ist eine Visualisierung des Bildes. Die farbigen Bänder repräsentieren die Bereiche, in denen a plus b gerade ist. Jeder Vektor (a; b), der auf diesen Bändern liegt, ist im Bild von T.")
]
]
]
...
#v(20pt)
#only("2-")[- Für $T(mat(x; y)) = mat(x + y; x - y)$, ist das Bild alle $mat(a; b)$, wobei $a + b$ gerade ist ➗2️⃣]
#v(20pt)
...
#only("2")[
#voiceover("Für die gegebene Abbildung T besteht das Bild aus allen Vektoren (a; b), wobei a plus b eine gerade Zahl ist.")
]
#only("3")[
