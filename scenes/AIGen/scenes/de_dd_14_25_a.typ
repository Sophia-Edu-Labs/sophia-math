...
#only("1")[
#voiceover("Die Aufgabe ist, ein Viertel plus zwei Fünftel zu berechnen.")
]
#only("2")[
...
#v(40pt)
#only("1-")[Dein Fehler: $1/4 + 2/5 ≠ (1+2)/(4+5)$]
#v(20pt)
...
#only("1")[
#voiceover("Du hast wohl einfach die Zähler addiert, und die Nenner addiert. Also eins plus zwei geteilt durch vier plus fünf. Das stimmt aber leider nicht.")
]
#only("2")[
...
#only("1")[
#voiceover("Hier siehst du die ursprünglichen Brüche. Oben haben wir ein Viertel, und unten sehen wir zwei Fünftel.")
]
#only("2")[
#voiceover("Diese Brüche haben verschiedene Nenner, also teilen wir sie in Stücke verschiedener Größe ein: Oben haben wir Viertel und rechts Fünftel. Deswegen können wir auch nicht einfach die Zähler addieren. Weil wir dann Stücke verschiedenr Größe addieren würden.")
]
#only("3")[
...
def visualize_equivalent_fractions(frac1, frac2):
    ...
    for i in range(1, 20):
        ax1.axvline(i / 20, color='black', linestyle='--', linewidth=6)
    ax1.add_patch(plt.Rectangle((0, 0), num1 / 20, 1, color='red', alpha=0.8))
    ax1.text(num1 / (2 * 20), 0.5, f'{num1}/{20}', ha='center', va='center', fontsize=33, color='white')
    ax1.set_xlim(0, 1)
    ax1.set_ylim(0, 1)
    ax1.set_xticks(np.arange(0, 1 + 1/20, 1/20))
    ax1.set_xticklabels([f'{i}/{20}' for i in range(20 + 1)])
    ax1.set_yticks([])

    for i in range(1, 20):
        ax2.axvline(i / 20, color='black', linestyle='--', linewidth=6)
    ax2.add_patch(plt.Rectangle((0, 0), num2 / 20, 1, color='blue', alpha=0.8))
    ax2.text(num2 / (2 * 20), 0.5, f'{num2}/{20}', ha='center', va='center', fontsize=33, color='white')
    ax2.set_xlim(0, 1)
    ax2.set_ylim(0, 1)
    ax2.set_xticks(np.arange(0, 1 + 1/20, 1/20))
    ax2.set_xticklabels([f'{i}/{20}' for i in range(20 + 1)])
    ax2.set_yticks([])
...
#only("2-")[#text()[- $5/20 = 1/4$]]
#v(20pt)
#only("3-")[#text()[- $8/20 = 2/5$]]
#only("1")[
#voiceover("Ein gemeinsamer Nenner wäre ja 20, denn 20 ist ein Vielfaches von 4 und 5.")
]
#only("2")[
#voiceover("Dann wandeln wir die Brüche um: Fünf Zwanzigstel ist gleich einem Viertel.")
]
#only("3")[
#voiceover("und acht Zwanzigstel ist gleich zwei Fünftel. Du siehst, dass auf jeder Seite der Wert gleich ist, wie davor. Lediglich die Darstellung ist anders.")
]
]
...
def visualize_sum_fraction(frac1, frac2):
    ...
    for i in range(1, 20):
        ax.axvline(i / 20, color='black', linestyle='--', linewidth=6)
    ax.add_patch(plt.Rectangle((0, 0), (num1 + num2) / 20, 1, color='purple', alpha=0.8))
    ax.text((num1 + num2) / (2 * 20), 0.5, f'{num1 + num2}/{20}', ha='center', va='center', fontsize=33, color='white')
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.set_xticks(np.arange(0, 1 + 1/20, 1/20))
    ax.set_xticklabels([f'{i}/{20}' for i in range(20 + 1)])
    ax.set_yticks([])
    plt.tight_layout()
    plt.show()
