...
#v(40pt)
Das Bild einer linearen Abbildung $T$ ist die Menge aller Vektoren $w$, so dass:
#v(40pt)
...
#only("4")[#voiceover("c) $w = 0$ ist ebenfalls falsch. Obwohl der Nullvektor Teil des Bildes sein könnte, ist er nicht das einzige Element. Das Bild kann viele andere Vektoren enthalten.")]
...
#v(20pt)
#only("2-")[Sei $mat(a; b)$ ein beliebiger Vektor in $RR^2$. Wir müssen $mat(x; y)$ finden, so dass $T(mat(x; y)) = mat(a; b)$.]
...
#only("2")[#voiceover("Um das Bild von $T$ zu finden, nehmen wir einen beliebigen Vektor $a, b$ in $RR^2$. Wir müssen einen Vektor $x, y$ finden, so dass $T$ von $x, y$ gleich $a, b$ ist.")]
...
#v(20pt)
#only("3-")[Also für jedes $mat(a; b)$ in $RR^2$ existiert ein $mat(x; y)$ so dass $T(mat(x; y)) = mat(a; b)$.]
...
#only("3")[#voiceover("Dies zeigt, dass wir für jeden Vektor $a, b$ in $RR^2$ einen Vektor $x, y$ finden können, so dass $T$ von $x, y$ gleich $a, b$ ist.")]
