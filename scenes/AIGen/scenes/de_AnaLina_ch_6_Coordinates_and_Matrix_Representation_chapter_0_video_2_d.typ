...
#only("1")[#voiceover("Leider, das war nicht korrekt. Hier ist das Video zur Erklärung.")]
// Erkläre kurz a und warum es falsch ist
#only("2")[#voiceover("a) Eine Linie ist falsch. Koordinaten identifizieren nicht eindeutig eine Linie in einem Vektorraum.")]
// Erkläre kurz b und warum es korrekt ist
#only("3")[#voiceover("b) Ein Punkt ist die richtige Antwort. Koordinaten identifizieren einen spezifischen Punkt in einem Vektorraum eindeutig.")]
...
#only("4-")[- Zum Beispiel in $RR^2$ mit der Standardbasis $mat(1;0), mat(0;1)$:]
#only("5-")[  - Der Punkt $mat(2;3)$ hat die Koordinaten $(2,3)$ 🔢]
#only("6-")[  - Das liegt daran, dass $2 dot mat(1;0) + 3 dot mat(0;1) = mat(2;3)$]
#only("7-")[- Also identifizieren Koordinaten einen Punkt im Vektorraum eindeutig ✅]
#v(40pt)
...
#only("6")[#voiceover("Das liegt daran, dass wenn wir 2, 3 als Linearkombination der Basisvektoren schreiben, wir 2 mal 1, 0 plus 3 mal 0, 1 erhalten, was 2, 3 ergibt.")]
...
#only("4-")[- Die Koordinaten $(2,3)$ identifizieren diesen Punkt im Vektorraum eindeutig 🎯]
...
#only("4")[#voiceover("Wie wir sehen können, identifizieren die Koordinaten 2, 3 diesen spezifischen Punkt im Vektorraum eindeutig. Kein anderer Punkt hat dieselben Koordinaten.")]
]
...
#only("5-")[Großartige Arbeit beim Verstehen dieses wichtigen Konzepts! 🌟]
...
#only("5")[#voiceover("Großartige Arbeit beim Verstehen dieses grundlegenden Konzepts der linearen Algebra! Mach weiter so.")]
]