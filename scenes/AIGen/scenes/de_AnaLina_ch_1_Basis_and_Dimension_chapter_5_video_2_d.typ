...
#v(40pt)
Die Koordinaten eines Vektors bezüglich einer Basis sind die \_\_\_\_\_ der linearen Kombination der Basisvektoren, die dem Vektor entspricht.
#v(40pt)
...
#only("-4")[d) Spann]#only("5-")[#text(fill:red)[d) Spann]]
...
#only("3")[#voiceover("Option b) Koeffizienten ist die richtige Antwort. Die Koordinaten sind tatsächlich die Koeffizienten der linearen Kombination der Basisvektoren, die dem gegebenen Vektor entspricht.")]
#only("4")[#voiceover("Option c) Dimensionen ist falsch. Dimensionen beziehen sich auf die Größe des Vektorraums, nicht auf die Koordinaten eines spezifischen Vektors.")]
#only("5")[#voiceover("Und Option d) Spann ist ebenfalls falsch. Spann bezieht sich auf die Menge aller linearen Kombinationen eines Satzes von Vektoren, nicht auf die Koordinaten eines spezifischen Vektors.")]
]

#slide()[
#text(size: 30pt, weight: "bold")[Lösung]
#v(40pt)
#only("1-")[Lassen Sie uns dies mit einem Beispiel verstehen 🧮]
#v(20pt)
#only("2-")[Betrachten Sie den Vektor $v=mat(3;4)$ und die Standardbasis $mat(1;0), mat(0;1)$ für $RR^2$.]
#v(20pt)
#only("3-")[Wir können $v$ als eine lineare Kombination der Basisvektoren schreiben:]
#v(20pt)
#only("4-")[$v = 3 mat(1;0) + 4 mat(0;1)$]
#v(20pt)
#only("5-")[Die Koeffizienten in dieser linearen Kombination, $3$ und $4$, sind die Koordinaten von $v$ bezüglich der Standardbasis 📐]

#only("1")[#voiceover("Lassen Sie uns dieses Konzept mit einem Beispiel verstehen.")]
#only("2")[#voiceover("Betrachten Sie den Vektor v gleich dem Vektor 3, 4 und die Standardbasisvektoren 1, 0 und 0, 1 für R².")]
#only("3")[#voiceover("Wir können v als eine lineare Kombination dieser Basisvektoren schreiben.")]
#only("4")[#voiceover("v ist gleich 3 mal der Vektor 1, 0 plus 4 mal der Vektor 0, 1.")]
#only("5")[#voiceover("Die Koeffizienten in dieser linearen Kombination, 3 und 4, sind die Koordinaten von v bezüglich der Standardbasis.")]
]
...
#only("4")[#voiceover("Diese Skalarmultiplikatoren, 3 und 4, sind die Koordinaten von v bezüglich der Basis e1 und e2.")]
]
]
...
#v(40pt)
#only("1-")[- Koordinaten sind die Koeffizienten in der linearen Kombination der Basisvektoren 🎯]
#v(20pt)
...
#only("1")[#voiceover("Zusammenfassend sind die Koordinaten eines Vektors bezüglich einer Basis die Koeffizienten in der linearen Kombination der Basisvektoren, die dem gegebenen Vektor entspricht.")]
