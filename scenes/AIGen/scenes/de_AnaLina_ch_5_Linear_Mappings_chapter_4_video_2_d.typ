...
#only("3")[#voiceover("Option b ist tatsächlich die korrekte Dimensionsformel. Sie besagt, dass die Dimension des Vektorraums V gleich der Summe der Dimensionen des Kerns und des Bildes von T ist.")]
...
#only("2")[#voiceover("Der erste Schritt besteht darin, sich an die Dimensionsformel zu erinnern. Sie besagt, dass die Dimension des Vektorraums V gleich der Summe der Dimensionen des Kerns und des Bildes von T ist.")]
]
...
#v(20pt)
- $"dim"(V)$: Dimension des Vektorraums
- $"dim"("ker"(T))$: Dimension des Kerns
- $"dim"("im"(T))$: Dimension des Bildes
#only("1")[#voiceover("Als nächstes identifizieren wir die Komponenten der Formel. dim V ist die Dimension des Vektorraums, dim ker T ist die Dimension des Kerns und dim im T ist die Dimension des Bildes.")]
#v(40pt)
#only("2-")[Daher lautet die korrekte Dimensionsformel:]
#v(20pt)
#text(weight: "bold")[b) $"dim"(V) = "dim"("ker"(T)) + "dim"("im"(T))$ ✅]
#only("2")[#voiceover("Daher ist die korrekte Antwort b. Die Dimension des Vektorraums V entspricht der Summe der Dimensionen des Kerns und des Bildes von T.")]
]