...
#only("1")[#voiceover("Nicht ganz. Schauen wir uns die richtige Lösung genauer an.")]
...
#only("3")[#voiceover("Das Addieren der Matrizen würde uns nicht die richtige Transformation geben.")]
#only("4")[#voiceover("Ebenso würde das Subtrahieren der Matrizen nicht das gewünschte Ergebnis liefern.")]
#only("5")[#voiceover("Und obwohl das Invertieren von Matrizen eine wichtige Operation ist, ist es hier nicht das, was wir für den Basiswechsel benötigen.")]
]
...
#v(20pt)
#only("2-")[1. Finden Sie die Matrixdarstellung von $T$ bezüglich der Standardbasis:]
#v(10pt)
#only("2-")[   $[T]_"standard" = mat(2, 0; 0, 3)$]
#v(20pt)
#only("3-")[2. Finden Sie die Basiswechselmatrix von der Standardbasis zu $B$:]
#v(10pt)
...
#v(40pt)
#only("1-")[3. Multiplizieren Sie die Matrizen, um die Matrixdarstellung von $T$ bezüglich $B$ zu erhalten:]
#v(10pt)
...
#v(20pt)
#only("3-")[- Neue Matrix stellt die lineare Abbildung in der neuen Basis dar 🆕]
#v(20pt)
#only("4-")[Großartige Arbeit! Sie haben ein wichtiges Konzept der linearen Algebra gemeistert. 🌟]

#only("1")[#voiceover("Zusammengefasst: Der Basiswechsel einer linearen Abbildung beinhaltet die Matrixmultiplikation.")]
#only("2")[#voiceover("Die ursprüngliche Matrixdarstellung wird durch die Basiswechselmatrix transformiert.")]
#only("3")[#voiceover("Die resultierende neue Matrix stellt die gleiche lineare Abbildung dar, jedoch nun bezüglich der neuen Basis.")]
#only("4")[#voiceover("Fantastische Arbeit beim Erfassen dieses wichtigen Konzepts. Machen Sie weiter so in Ihrer linearen Algebra-Reise!")]
]