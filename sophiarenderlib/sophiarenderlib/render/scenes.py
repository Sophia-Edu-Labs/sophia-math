...
# A method that will create the output file name given a scene class and a locale
def create_output_file_name(sc: Type[SophiaScene], parent_output_dir: Path, include_locale_in_file: bool = True):
    ...
    output_path = (parent_output_dir/sc.__module__/f"{locale_prefix}{sc.__name__}.mp4").resolve()
...
def render_scene(sc: Type[SophiaScene], output_dir: Path, scene_media_folder: Path, test_run_with_last_frame: bool = False, disable_progressbar: bool = False, quality: str = "production_quality", include_locale_in_file: bool = True):
    if test_run_with_last_frame:
        ...
        output_path = create_output_file_name(sc, output_dir, include_locale_in_file)
